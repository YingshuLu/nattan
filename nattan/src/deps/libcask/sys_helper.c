...
int co_read_till(int fd, char *buf, unsigned int buf_len, time_t timeout_ms) {
    if (fd < 0 || !buf || buf_len <= 0) return -1;

    struct pollfd p;
    p.fd = fd; 
    p.events = EPOLLIN | EPOLLRDHUP | EPOLLERR;

    co_disable_hook();
    int ret = read(fd, buf, buf_len);
    if(ret == -1 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
        ret = co_poll(&p, 1, timeout_ms);
        if (ret < 0) return -1;
...
int co_write_till(int fd, const char *buf, unsigned int buf_len, time_t timeout_ms) {
    if (fd < 0 || !buf || buf_len <= 0) return -1;
